...
import (
	...
	storedto "rakamin-mini-project/internal/pkg/dto_store"
...
type BooksUseCase interface {
	GetAllBooks(ctx context.Context, params storedto.BooksFilter) (res []storedto.BooksResp, err *helper.ErrorStruct)
	GetBooksByID(ctx context.Context, booksid string) (res storedto.BooksResp, err *helper.ErrorStruct)
	CreateBooks(ctx context.Context, data storedto.BooksReqCreate) (res uint, err *helper.ErrorStruct)
	UpdateBooksByID(ctx context.Context, booksid string, data storedto.BooksReqUpdate) (res string, err *helper.ErrorStruct)
...
func (alc *BooksUseCaseImpl) GetAllBooks(ctx context.Context, params storedto.BooksFilter) (res []storedto.BooksResp, err *helper.ErrorStruct) {
	if params.Limit < 1 {
		params.Limit = 10
	}
	...
	for _, v := range resRepo {
		res = append(res, storedto.BooksResp{
			ID:          v.ID,
			Title:       v.Title,
...
func (alc *BooksUseCaseImpl) GetBooksByID(ctx context.Context, booksid string) (res storedto.BooksResp, err *helper.ErrorStruct) {
...
func (alc *BooksUseCaseImpl) CreateBooks(ctx context.Context, data storedto.BooksReqCreate) (res uint, err *helper.ErrorStruct) {
...
func (alc *BooksUseCaseImpl) UpdateBooksByID(ctx context.Context, booksid string, data storedto.BooksReqUpdate) (res string, err *helper.ErrorStruct) {
	if errValidate := helper.Validate.Struct(data); errValidate != nil {
		log.Println(errValidate)
		return res, &helper.ErrorStruct{
			Err:  errValidate,
