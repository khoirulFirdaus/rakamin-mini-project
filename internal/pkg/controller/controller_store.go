...
import (
	"log"
	storedto "rakamin-mini-project/internal/pkg/dto"
...
func (uc *BooksControllerImpl) GetAllBooks(ctx *fiber.Ctx) error {
	...
	filter := new(storedto.BooksFilter)
	...
	res, err := uc.booksusecase.GetAllBooks(c, storedto.BooksFilter{
		Title: filter.Title,
		Limit: filter.Limit,
		Page:  filter.Page,
	})
...
func (uc *BooksControllerImpl) CreateBooks(ctx *fiber.Ctx) error {
	...
	data := new(storedto.BooksReqCreate)
...
func (uc *BooksControllerImpl) UpdateBooksByID(ctx *fiber.Ctx) error {
	...
	data := new(storedto.BooksReqUpdate)
	if err := ctx.BodyParser(data); err != nil {
		return ctx.Status(fiber.StatusBadRequest).JSON(fiber.Map{
			"error": "Bad request",
		})
